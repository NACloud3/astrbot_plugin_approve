...
插件会读取 OneBot `aiocqhttp` 的入群申请事件，把验证信息的完整内容作为 Minecraft Java 版 ID，并调用 Mojang Username -> UUID 接口验证该 ID 是否存在。
...
- `reject_when_no_username`：申请内容不是完整 ID 时是否拒绝。
- `reject_reason`：申请内容不是完整 ID，或查询不到该 ID 时的拒绝理由。
